...
int main( int argc, char *argv[] ) {
	...
	while ( rp != NULL ) {
		...
		bindval = bind(sfd, (struct sockaddr *)rp->ai_addr, rp->ai_addrlen);
		if ( bindval != -1 ) {
			break;
		}
	...
	if ( (cfd = accept(sfd, (struct sockaddr *)&claddr, &slen)) == -1 ) {
		fprintf(stderr, "error in accept(): %s\n", strerror(errno));
		return -1;	
	} else {
		fprintf(stdout, "successfully accepted\n");
	}
